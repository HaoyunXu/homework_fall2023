...
class AWACAgent(DQNAgent):
    ...
    def compute_critic_loss(
        ...
        with torch.no_grad():
            ...
            next_qs = next_qa_values.gather(dim=-1, index=next_action_dist.sample().unsqueeze(-1)).squeeze(-1)

            # TODO(student): Compute the TD target
            target_values = rewards + self.discount * next_qs * torch.logical_not(dones)
    ...
    def compute_advantage(
        ...
        probs = nn.functional.softmax(action_dist.logits, dim=-1)
    ...
    def update_actor(
        ...
        action_dist = self.actor(observations)
        action_probs = action_dist.log_prob(actions).unsqueeze(-1)
        loss = - (weights * action_probs).mean()

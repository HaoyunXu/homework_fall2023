...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            assert next_qs.shape[0] == 2, next_qs.shape
            # Swap predictions between critics while maintaining shape
            next_qs = next_qs[[1, 0]]  # directly swap the critics' predictions
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward + \
                self.discount * next_qs * torch.logical_not(done)
        ...
        assert q_values.shape == (
            self.num_critic_networks, batch_size), q_values.shape
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        If distribution has no entropy() method or raises NotImplementedError,
        approximate it with: H[p] ≈ -E[log p(x)] where x ~ p(x)
        """
        try:
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            advantage = q_values
            assert advantage.shape == (
                self.num_actor_samples, batch_size), advantage.shape
